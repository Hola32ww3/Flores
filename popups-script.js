...
function blockPopupsFunc({ iframeId, showNotification, popupResources = {} }) {
  ...
  const popupAllowedRegex =
    /^(http(s)?:)?\/\/([^/]*\.)?(pinterest\.com|paid\.outbrain\.com|twitter\.com|paypal\.com|yahoo\.com|facebook\.com|linkedin\.com|salesforce\.com|amazon\.co|google\.co|aol\.com|aol\.de)/i;
  const popupAllowHosts =
    /^http(s):\/\/([^/]*\.)?(search\.yahoo\.com|linkedin\.com|facebook\.com|google\.com|aol\.com|aol\.de)/i;
  const anchorPopupsExcludedHosts = {
    "sh.st": true,
  };
  const popupDeniedRegexs = [
    "directrev\\.com",
    "tradeadexchange\\.com",
    "liveadexchanger\\.com",
    "interyield\\.",
  ...
  window.open = function (...args) {
    ...
    const { target, currentTarget } = window.event;
    ...
    const targetValid =
      target?.tagName === "A" && target.href.startsWith("http");
  ...
  document.createElement = function (tagName, options) {
    const element = stndz.originalDocumentCreateElement.apply(document, [
      tagName,
      options,
    ]);
      ...
      element.addEventListener(
        ...
        (event) => {
          ...
          if (anchorPopupsExcludedHosts[document.location.host]) {
            ...
            const isNewExternalLink =
              now - createTime < 50 &&
              block === IS_POPUP_RESULT.unknown &&
              !window.location.hostname.includes(link.hostname);
            if (block === IS_POPUP_RESULT.yes || isNewExternalLink) {
              event.preventDefault();
              showPopupNotificationWindow("create-link", link.href, [], () =>
                element.click()
              );
            }
          }
      ...
      switch (event.data.type) {
        case POPUP_MESSAGE_TYPES.stndzShowPopupNotification:
          if (
            window !== window.top ||
            !stndz.active ||
            event.data.payload.iframeId !== iframeId
          ) {
            return;
          }
          ...
          if (stndz.popupNotificationOpen === null) {
            ...
            stndz.showPopupNotification = function () {
              ...
              const hidePopupNotificationId = window.setTimeout(
                stndz.hidePopupNotification,
                30 * 1000
              );
  ...
  function showPopupNotificationWindow(blockType, popupUrl, args, clicker) {
    const openPopupFunc = () =>
      clicker ? clicker() : stndz.originalWindowOpen.apply(window, args);
    if (!stndz.active) {
      openPopupFunc();
    }
    if (!showNotification) {
  ...
  function createNotificationOnPage() {
    ...
    for (const { id, action } of handlers) {
  ...
  document.addEventListener("sendToPopupBlocking", (e) => {
    ...
    if (
      detail.type === POPUP_MESSAGE_TYPES.stndzPopupUpdate &&
      iframeId === detail.payload.iframeId &&
      typeof detail.payload.active === "boolean"
    ) {
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      const isSuspiciousLink =
        target.tagName === "A" && target.onclick && target.target === "_blank";
      if (isSuspiciousLink) {
        e.preventDefault();
